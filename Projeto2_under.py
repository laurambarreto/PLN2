...
# Número fixo de amostras por classe
n_amostras = 500

# Aplicar undersampling
train_df = train_df.groupby('classe').apply(lambda x: x.sample(n = min(len(x), n_amostras), random_state = 42)).reset_index(drop = True)

# Contagem de amostras por classe nos dados de treino após Undersampling
print(f"\nDados de treino após Undersampling:", train_df["classe"].value_counts())
...
def regressao_logistica ():
    ...
    log_reg.fit (X_train_num, y_train_num)
...
def naive_bayes ():
    nb = MultinomialNB()
    nb.fit(X_train_num, y_train_num)
