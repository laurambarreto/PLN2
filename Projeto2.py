...
X_ngrams_train = vectorizer.fit_transform(train_df["tokens"].apply(lambda tokens: ' '.join(tokens)))
X_ngrams_test = vectorizer.transform(test_df["tokens"].apply(lambda t: ' '.join(t)))
...
train_df = pd.concat([train_df, train_counts_exclusivas, train_counts_adj_adv], axis = 1)
test_df = pd.concat([test_df, test_counts_exclusivas, test_counts_adj_adv], axis = 1)
...
# Descobrir quantos clusters o K-Means realmente criou
num_clusters = len(np.unique(y_pred_kmeans))
print(f"\nNúmero de clusters encontrados: {num_clusters}")

# Criar nomes automáticos (caso o KMeans crie 4 ou mais)
target_names_auto = [f"Cluster {i}" for i in range(num_clusters)]

print("-------- MODELO K-MEANS --------")
print("Accuracy:", accuracy_score(y_test, y_pred_kmeans))
print("\nRelatório de classificação:")
print(classification_report(test_df ['classe'], y_pred_kmeans, target_names = ['Citação (-1)', 'Facto (0)', 'Viés (1)']), target_names = target_names_auto)
print()
